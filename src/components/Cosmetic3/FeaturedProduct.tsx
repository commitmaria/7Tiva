...
const FeaturedProduct: React.FC<Props> = ({ data }) => {
  ...
  const totalAmount = productMain.price * productMain.quantityPurchase;
          ...
          <div className="product-infor md:w-1/2 w-full lg:pl-16 md:pl-6">
            ...
            <div className="list-action mt-6">
              <div className="choose-size">
                <div className="heading flex items-center justify-between">
                  <div className="text-title">
                    Volume:{" "}
                    <span className="text-title size">{`${activeSize}ml`}</span>
                  </div>
                </div>
                <div className="list-size flex items-center gap-2 flex-wrap mt-3">
                  {productMain.sizes.map((item, index) => (
                    <div
                      className={`size-item w-[72px] h-12 flex items-center justify-center text-button rounded-lg bg-white border border-line ${
              ...
              <div className="button-block mt-5">
                {/* PayPal Button: charges dynamically based on quantity */}
                {productMain && (
                  <PayPalButton
                    amount={totalAmount} // dynamic price × quantity
