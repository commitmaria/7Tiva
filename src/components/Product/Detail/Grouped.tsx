...
const Grouped: React.FC<Props> = ({ data, productId }) => {
  ...
  // compute the same slice of grouped items you render below
  const groupedItems = data.slice(Number(productId), Number(productId) + 3);

  // compute total price dynamically using quantity state (fallback to 1)
  const totalPrice = groupedItems.reduce((sum, it) => {
    const qty = quantity[it.id] || 1; // fallback to 1 if not set
    return sum + Number(it.price) * qty; // ensure numeric add
  }, 0);
